<template>
  ...
  >
    <template #title>
      <strong>
        {{ t("07.surveySatisfactionOffCampus.callSurvey.pageTitle") }}</strong
      >
    </template>
    <template #default>
...
export default {
  ...
  data() {
    return {
      ...
      searchModel: {} as SurveyOutSchoolSearchPopupModel,
      rowData: [] as Array<SurveyOutSchoolListPopupModel>,
      totalRows: 0,
      key: 1,
    };
  },
  ...
  methods: {
    ...
    handleCustomClick(data: SurveyOutSchoolListPopupModel) {
      this.$swal({
    ...
    handleChooseSurvey(data: SurveyOutSchoolListPopupModel) {
      this.cmn.setLoading(true);
