          ...
          <table class="tbl_row tbl_border">
            ...
            <tbody>
              ...
              <tr>
                <th scope="row">
                  <p class="required">
                    {{ t("07.surveySatisfactionOffCampus.crud.surveyUrl") }}
                  </p>
                </th>
                <td class="td_input">
                  <span v-if="modeScreen !== modeDetail" class="text-gray">
                    {{ t("07.surveySatisfactionOffCampus.crud.planUrl") }}</span
                  >
                  <div v-else>
                    <span class="text-gray">{{ dataForm.surveyUrl }}</span>
                    <button
                      type="button"
                      @click="copyUrlSurvey"
                      class="btn_round btn_xs btn_lightgray ml_5"
                    >
                      {{ t("07.surveySatisfactionOffCampus.crud.btnCopyUrl") }}
                    </button>
                  </div>
                </td>
              </tr>
      ...
      <div class="box dp_block">
        <div class="box_section">
          ...
          >
            ...
            <div>
              <button
                class="btn_round btn_xs btn_secondary mx-2"
                @click="openModalCall()"
                :disabled="isDisabled"
              >
                {{ t("07.surveySatisfile.formAdd.button1") }}
              </button>
              <button
                    ...
                    <td>
                      <div class="flex-custom">
                        <button
                          ...
                          "
                          class="button btn_xs btn_white mx-1"
          ...
          <div class="box_section">
            <div class="dp_flex btn_group btn_end mt_8" style="gap: 10px">
              ...
              >
                {{ t("07.surveySatisfactionOffCampus.crud.deleteSurvey") }}
              </button>
              <button
                type="button"
                class="btn_round btn_xs btn_primary"
...
import {
  deleteSurveyOutSchool,
  detailSurveyOutSchool,
  saveSurveyOutSchool,
} from "../../stores/survey/outSchool/surveyOutSchool.service";
import CallSurvey from "./CallSurvey.vue";
import {
  SurveyOutSchoolQuestionModel,
...
export default {
  ...
  data() {
    return {
      pageTitle: this.t("07.surveySatisfactionOffCampus.crud.pageTitle"),
      statusN: STATUS_NO,
      statusY: STATUS_YES,
      id: "",
      modeScreen: "" as any,
      ...
      dataForm: {} as SurveyOutSchoolReqModel,
      qusTypeScore5: QUS_TYPE_SCORE_5,
      qusType03: QUS_TYPE_03,
      qusType02: QUS_TYPE_02,
      isDisabled: false,
      domainClient:
        import.meta.env.VITE_CLIENT_ENDPOINT +
        SCREEN.surveySatisfactionOffCampus.path,
      modules: {
        name: "imageUploader",
          ...
          upload: (file: any) => {
            return new Promise((resolve, reject) => {
              if (!file) {
                ...
                formData.append("category", "SURVEY_OUT_SCHOOL_EDITOR");
                formData.append("orgName", "SURVEY_OUT_SCHOOL_EDITOR");
                formData.append("sectionName", "SURVEY_OUT_SCHOOL_EDITOR");
  ...
  methods: {
    getDataDetail() {
      this.cmn.setLoading(true);
      detailSurveyOutSchool({ diagSeq: this.id }).then((res) => {
    ...
    saveData() {
      this.cmn.setLoading(true);
      saveSurveyOutSchool(this.dataForm)
        .then((res) => {
          this.$swal({
    ...
    deleteSurvey() {
      this.cmn.setLoading(true);
      deleteSurveyOutSchool({ diagSeq: this.id })
        .then((res) => {
          this.$swal({
    ...
    back() {
      this.$router.push({ path: SCREEN.surveySatisfactionOffCampus.path });
    },
    ...
    copyUrlSurvey() {
      ...
      urlInput.value = this.dataForm.surveyUrl;
      urlInput.select();
      document.execCommand("copy");
      document.body.removeChild(urlInput);
      this.$swal({
        text: this.t("common.message.copyUrl"),
        type: "success",
        showCancelButton: true,
        showConfirmButton: false,
        cancelButtonText: this.t("common.confirm"),
