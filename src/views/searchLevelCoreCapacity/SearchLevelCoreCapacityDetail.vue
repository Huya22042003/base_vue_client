      ...
      <div class="box dp_block">
        <div class="box_section">
          <p class="box_title_sm">
            {{ t("07.searchLevelCapacity.detail.title1") }}
          </p>
          <div class="scrollx_table lg">
            <table class="tbl_col">
              ...
              <thead>
                <tr>
                  <th scope="col">
                    {{ t("07.searchLevelCapacity.detail.university") }}
                  </th>
                  <th scope="col">
                    {{ t("07.searchLevelCapacity.detail.department") }}
                  </th>
                  <th scope="col">
                    {{ t("07.searchLevelCapacity.detail.nameStd") }}
                  </th>
                  <th scope="col">
                    {{ t("07.searchLevelCapacity.detail.idStd") }}
                  </th>
                  <th scope="col">
                    {{ t("07.searchLevelCapacity.detail.gender") }}
                  </th>
                  <th scope="col">
                    {{ t("07.searchLevelCapacity.detail.eduYear") }}
                  </th>
                  <th scope="col">
                    {{ t("07.searchLevelCapacity.detail.status") }}
                  </th>
                  <th scope="col">
                    {{ t("07.searchLevelCapacity.detail.studyYn") }}
                  </th>
                </tr>
              </thead>
              <tbody>
        ...
        <div class="box_section">
          <p class="box_title_sm">
            {{ t("07.searchLevelCapacity.detail.title3") }}
          </p>
          <div class="scrollx_table lg">
            <table class="tbl_col">
              <caption>
              ...
              <thead>
                <tr>
                  <th scope="col">
                    {{ t("07.searchLevelCapacity.table.eduYear") }}
                  </th>
                  <th scope="col">
                    {{ t("07.searchLevelCapacity.detail.name") }}
                  </th>
                  <th
                    scope="col"
                    v-for="data in listName"
                    :key="data.coreAbilitySeq"
                  >
                    {{ data.coreAbilityNm }}
                  </th>
                </tr>
              ...
              <tbody>
                <tr v-for="data in detailDataList" :key="data.id">
                  ...
                  <td v-for="name in listName" :key="name.coreAbilitySeq">
                    {{ data.scores[name.coreAbilitySeq] || 0 }}
                  </td>
                </tr>
              </tbody>
            </table>
...
export default defineComponent({
  ...
  data() {
    return {
      pageTitle: this.t("07.searchLevelCapacity.detail.pageTitle"),
      dataLineChart: {
        dataset: [],
        color: ["#546E7A", "#E91E63"],
        labelsPosition: {
  ...
  beforeMount() {
    ...
    try {
      Promise.all([this.getDetailData(), this.getCoreAbilityName()]);
    } catch (e) {
      alert(e);
    }
  },
  methods: {
        ...
        return this.detailDataList.map((data) => {
          return {
            ...
            data: this.listName.map((name) => {
              const originalValue = data.scores[name.coreAbilitySeq] || 0;
    ...
    getChartCategories() {
      if (Array.isArray(this.listName) && this.listName.length > 0) {
        return this.listName.map((element) => element.coreAbilityNm);
      }
      return [];
    },
