                ...
                <tr>
                        ...
                        <ul>
                          <template v-if="listClassificationSystem">
                            <li class="mt-1">
                              <input
                                type="checkbox"
                                :id="listClassificationSystem.id.toString()"
                                :name="listClassificationSystem.id.toString()"
                                checked
                              />
                              <!--                            <input type="checkbox" :id="item.id" :name="item.id"/>-->
                              <label
                                :for="listClassificationSystem.id.toString()"
                                :class="{
                                  active:
                                    active === listClassificationSystem.id,
                                }"
                                @click.stop="
                                  handleClickActive(
                                    listClassificationSystem.id.toString(),
                                    listClassificationSystem.module.toString(),
                                    0,
                                    listClassificationSystem.useYn
                                  )
                                "
                              >
                                <img
                                  src="../../assets/images/icon_disk.svg"
                                  alt="disk"
                                />
                                <span>{{
                                  listClassificationSystem.module
                                }}</span>
                              </label>
                              <ul class="child-list">
                                <template
                                  v-for="itemChild in listClassificationSystem.child"
                                  :key="itemChild.jobCapaStndSeq"
                                >
                                  <li>
                                    <input
                                      type="checkbox"
                                      :id="itemChild.jobCapaStndSeq"
                                      :name="itemChild.jobCapaStndSeq"
                                    />
                                    <label
                                      :for="itemChild.jobCapaStndSeq"
                                      :class="{
                                        active:
                                          active === itemChild.jobCapaStndSeq,
                                      }"
                                      @click.stop="
                                        handleClickActive(
                                          itemChild.jobCapaStndSeq,
                                          itemChild.jobCapaStndNm,
                                          1,
                                          itemChild.useYn
                                        )
                                      "
                                    ...
                                    <ul class="child-list">
                                      <template
                                        ...
                                        :key="itemChild1.jobCapaStndSeq"
                                      >
                                        <li>
                                          <input
                                            type="checkbox"
                                            :id="itemChild1.jobCapaStndSeq"
                                            :name="itemChild1.jobCapaStndSeq"
                                          />
                                          <label
                                            :for="itemChild1.jobCapaStndSeq"
                                            :class="{
                                              active:
                                                active ===
                                                itemChild1.jobCapaStndSeq,
                                            }"
                                            @click.stop="
                                              handleClickActive(
                                                itemChild1.jobCapaStndSeq,
                                                itemChild1.jobCapaStndNm,
                                                2,
                                                itemChild1.useYn
                                              )
                                            "
                                          ...
                                          <ul class="child-list">
                                            <template
                                              v-for="itemChild2 in itemChild1.child"
                                            ...
                                            >
                                              <li>
                                                ...
                                                >
                                                  <template
                                                    v-for="itemChild3 in itemChild2.child"
                                                    :key="
                                                      itemChild3.jobCapaStndSeq
                                                    "
                                                  >
                                                    <li>
                                                      <input
                  ...
                  <td>
                    <div class="scrollable list-other">
                      <ul>
                        <li
                          v-for="item in listUnitCapacity"
                          :key="item.jobAbilSeq"
                          @click.stop="
                            handleActive(item.jobAbilSeq, item.usedStatus)
                          "
                        >
                  ...
                  <td>
                    <div v-if="activeUsed">
                      <p
                        class="font_16 normal mb_2"
                        :style="{ color: activeUsed === 'Y' ? '' : 'red' }"
                      >
                        {{ activeUsed === "Y" ? "사용" : "미사용" }}
                      </p>
                    </div>
...
onMounted(() => {
  active.value = listSelectBox.value.filter(
    (item) => item.cdId === selectBoxValue.value
  )[0].cdId;
  getData();
});
...
const getData = async () => {
  ...
  if (lmStore.learningList && lmStore.status == SUCCSESS_STATUS) {
    listClassificationSystem.value = {
      id: listSelectBox.value.filter(
        (item) => item.cdId === selectBoxValue.value
      )[0].cdId,
      module: listSelectBox.value.filter(
        (item) => item.cdId === selectBoxValue.value
      )[0].cdNm,
...
const handleClickActive = async (
  ...
  const checkUnit =
    listUnitCapacity.value.find((item) => item.id === activeModule?.value) ||
    null;
...
const handleRegister = async (value: string) => {
  ...
  await lmStore.addModuleList({
    jobCapaStndNm: value,
    upJobCapaStndSeq:
      active.value === CODE_NCS || active.value === CODE_KCS
        ? null
        : active.value,
    divCd: selectBoxValue.value,
  });
