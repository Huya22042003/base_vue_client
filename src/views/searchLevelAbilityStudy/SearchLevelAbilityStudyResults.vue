            ...
            <table class="tbl_row tbl_border">
              ...
              <tbody>
                <tr v-for="(data, index) in element()" :key="index">
                  ...
                  <td v-for="itemName in data.name" :key="itemName.name">
                    {{ itemName.name }}
                  </td>
                </tr>
              </tbody>
...
import { getListResult } from "@/stores/searchLevelAbilityStudy/searchLevelAbilityStudy.service";
...
export default defineComponent({
  ...
  data() {
    return {
      ...
      resultList: [] as Array<LearnModel>,
    ...
    element() {
      ...
      this.resultList.forEach((item1) => {
        item1.chldList.forEach((item2, index2) => {
          let listName = [];
          let listPoint = [];
          item2.learnComponentList.forEach((item3) => {
            listName.push({ name: item3.learnComponentNm || "" });
          });
          ...
          let elName = {
            ...
            isIndex: index2 == 0,
            isIndex2: true,
            nameParentItem1: item1.learnAbilityNm,
