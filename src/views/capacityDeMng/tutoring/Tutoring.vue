...
import type {
  TutoringFilterDTO,
  TutoringResDTO,
} from "@/stores/capacityDeMng/mentoring/tutoring/tutoring.type";
import { getPageTutoring } from "@/stores/capacityDeMng/mentoring/tutoring/tutoring.service";
import {
  CAMPUS_UP_CD_ID,
...
export default defineComponent({
  ...
  data() {
    return {
      pageTitle: this.t("capacityDeMng.tutoring.breadcrumb.pageTitle"),
      ...
      columnDefs: ref([
        ...
        },
        {
          ...
          cellRendererParams: (data: TutoringResDTO) => {
            return {
              listParams: data.report,
  ...
  methods: {
    ...
    getAllData() {
      this.store.setLoading(true);
      getPageTutoring(this.dataSearch)
        .then((res) => {
          if (res.status === SUCCSESS_STATUS) {
            this.totalRecord = res.data.data.totalElements;

            this.data = res.data.data.content.map((el: any) => {
              el.report = this.convertPlan(el.report);

              el.mentorNm = el.mentorNm ? `${this.convertUser(
                el.mentorNm
              )}(${this.convertUser(el.stdMentor)})` : "";

              el.menteeNm =  el.menteeNm ? `${this.convertUser(
                el.menteeNm
              )}(${this.convertUser(el.stdMentee)})` : "";
