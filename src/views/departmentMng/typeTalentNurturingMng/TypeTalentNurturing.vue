<template>
            ...
            <div class="search_box">
                <div class="search_section">
                    <div class="search_toggle sm">
                        <div class="left"></div>
                        <div class="right">
                            <div class="search_daywrap mg_r20">
                                <span class="tag">
                                    {{ t('departmentMng.typeTalentNurturingMng.search.dept') }}
                                </span>
                                <div class="select_row">
                                    <SelectBoxBaseSearch :id="'selectbox'" :name="'selectbox'" v-model="dataSearch.dept"
                            ...
                            <div class="search_daywrap mg_r10">
                                ...
                                <div class="select_row">
                                    <InputBase v-model="dataSearch.typeTalent" id="input_typeTalent" />
                                </div>
                            </div>
                            <button type="button" class="btn_round btn_lg btn_primary mg_l10" @click="searchFormData">조회하기</button>
                            <button type="button" class="btn_round btn_lg btn_gray mg_l5" @click="clearFormSearch">초기화</button>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <template #button>
                        <div>
                            <button class="btn_round btn_sm btn_primary" @click="create">
                                <!-- 등록 -->
                                {{ t('departmentMng.typeTalentNurturingMng.list.btnCreate') }}</button>
                        </div>
                    </template>
                </GridComponentV2>
            </div>
    ...
    <TypeTalentNurturingForm v-if="modalOpen" :is-open="modalOpen" :id="id" @close-modal="closeModal"></TypeTalentNurturingForm>
</template>
...
export default {
    ...
    data() {
        return {
            ...
            breadcrumbItems: [
                /* 'HOME' */
                { label: this.t('departmentMng.typeTalentNurturingMng.breadcrumb.01'), link: '/' },
                /* '학과정보 관리' */
                { label: this.t('departmentMng.typeTalentNurturingMng.breadcrumb.02'), link: '#' },
                /* '인재양성유형 및 직무 관리' */
                { label: this.t('departmentMng.typeTalentNurturingMng.breadcrumb.03'), link: SCREEN.typeTalentNurturingCrudDept.path }
            ],
            ...
            columnDefs: ref([
                {
                    headerName: this.t('departmentMng.typeTalentNurturingMng.list.dept')/* '학과' */,
                    field: 'deptmentNm',
                    cellStyle: {
                        textAlign: 'center'
                    }
                ...
                {
                    headerName: this.t('departmentMng.typeTalentNurturingMng.list.typeTalent')/* '인재양성유형' */,
                    field: 'typeNm',
                ...
                {
                    headerName: this.t('departmentMng.typeTalentNurturingMng.list.useYn')/* '학과' */,
                    field: 'useYn',
                    cellStyle: {
                        textAlign: 'center'
                    },
                ...
                {
                    headerName: this.t('departmentMng.typeTalentNurturingMng.list.createdBy')/* '등록자' */,
                    field: 'regId',
                    cellStyle: {
                        textAlign: 'center'
                    },
                ...
                {
                    headerName: this.t('departmentMng.typeTalentNurturingMng.list.createdDate')/* '등록일' */,
                    field: 'regDate',
                    cellStyle: {
                        textAlign: 'center'
                    },
