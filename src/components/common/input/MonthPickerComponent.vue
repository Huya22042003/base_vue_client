<template>
  <div class="relative" ref="calendarParent">
    <input v-if="(mode == modeShow || mode == '') && !isDisable" type="text" :id="id" :name="name"
      :placeholder="placeholder" :value="selectedDateText" @click="showCalendar" @input="updateParent"
      @change="updateParent" class="datepick" />
        ...
        <div class="datepicker-years">
          <table class="table-condensed">
            ...
            <tbody>
              <tr>
                <td colspan="7">
                  <span class="year" v-for="year in yearRange" :key="year" @click="selectYearChoose(year)" :class="{
...
export default {
  ...
  setup(props) {
    ...
    watch(() => props.modelValue, (newValue, oldValue) => {
      if (newValue) {
        ...
        selectedDateText.value = format(props.modelValue, 'yyyy-MM');
      }
      else {
  ...
  mounted() {
    if (this.modelValue) {
      this.selectedDateText = format(this.modelValue, 'yyyy-MM');
  ...
  // watch :{
  //   selectedDateText : function(newVal,olVal) {
  //     this.checkData();
  //   }
  // },
  methods: {
    ...
    selectDay(day) {
      ...
      this.selectedDateText = `${day.date.getFullYear()}-${month}`
